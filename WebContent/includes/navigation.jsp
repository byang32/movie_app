		<ul class="nav nav-tabs">
			<li><a href="index.jsp">Home</a><li>
			<li class="dropdown">
				<a href="#" class="dropdown-toggle" data-toggle="dropdown">View All<b class="caret"></b></a>
				<ul class="dropdown-menu">
					<li><a href="ViewAll">Unsorted</a>
					<li><a href="ViewAll?sortType=title">by Title</a>
					<li><a href="ViewAll?sortType=director">by Director</a>
					<li><a href="ViewAll?sortType=lengthInMinutes">by Length of Movie</a>
				</ul>
			</li>
			<li><a href="search.jsp">Title Search</a></li>
			<li><a href="searchDirector.jsp">Director Search</a>
			<li><a href="addMovies.jsp">Add Movies</a></li>
			<li><a href="populateDatabase.jsp">Populate Database</a></li>
		</ul>